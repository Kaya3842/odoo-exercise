...
class Lead(models.Model):
    ...
    expected_revenue = fields.Monetary('Expected Revenue', currency_field='company_currency', tracking=True, 
                                        store=True, compute="_compute_expected_revenue") # Changed to computed field
    ...
    # New compute methods
    @api.depends("request_ids", "request_ids.qty")
    def _compute_total_sales(self):
        for lead in self:
            if not lead.request_ids:
                lead.total_sales = 0
            total = sum(request.qty for request in lead.request_ids)
            lead.total_sales = total
    ...
    def _compute_expected_revenue(self):
        for lead in self:
            ...
            revenue = sum(request.qty * request.product_id.list_price for request in lead.request_ids)
